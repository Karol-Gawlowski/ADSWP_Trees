...
for (i in 1:(CV-1)){
  ...
  holdout_rows = which(CV_vec == max(CV))
  
  iter = paste0("CV_",i)
  
  models[[iter]] = list()
  
  results[[iter]] = data.frame(ID = dt_list$fre_mtpl2_freq$IDpol[holdout_rows],
                               actual = dt_list$fre_mtpl2_freq$ClaimNb[holdout_rows],
                               glm = NA,
                               SAV_glm = NA,
                               XGB = NA,
                               train_GLM_w_XGB = NA,
  ...
  results[[iter]]$glm = as.vector(predict(models[[iter]]$glm_model,
                                          dt_list$fre_mtpl2_freq[holdout_rows,-c(1,3)],type="response"))
  ...
  results[[iter]]$SAV_glm = predict(models[[iter]]$SAV_glm_model,dt_list$fre_mtpl2_freq[holdout_rows,-c(1,3)])
  ...
  results[[iter]]$XGB = predict(models[[iter]]$XGB_model,
                                xgb.DMatrix(data.matrix(dt_list$fre_mtpl2_freq[holdout_rows,-c(1,2,3)])))
  ...
  results[[iter]]$train_GLM_w_XGB = predict(models[[iter]]$train_GLM_w_XGB,
                                            dt_list$fre_mtpl2_freq[holdout_rows,-c(1,2,3)])
  ...
  models[[iter]]$XGB_init_GLM_model = train_XGBoost(glm_model = models[[iter]]$glm_model, dt = dt_list$fre_mtpl2_freq[train_rows,-c(1,2,3)],
                                                    y = dt_list$fre_mtpl2_freq$ClaimNb[train_rows],
                                                    vdt = list(x_val = dt_list$fre_mtpl2_freq[valid_rows,-c(1,2,3)],
                                                               y_val = dt_list$fre_mtpl2_freq$ClaimNb[valid_rows]), use_glm= TRUE
  )
  
  dval_with_margin = xgb.DMatrix(data.matrix(dt_list$fre_mtpl2_freq[holdout_rows,-c(1,2,3)]))
  val_base_margin = predict(models[[iter]]$glm_model, dt_list$fre_mtpl2_freq[holdout_rows,-c(1,2,3)],type="link")
